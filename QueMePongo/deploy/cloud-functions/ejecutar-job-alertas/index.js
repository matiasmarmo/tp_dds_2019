...
const vmConfig = {
    ...
    "metadata": {
        ...
        "items": [
            {
                "key": "startup-script",
                "value": "sudo apt install default-jre\ngsutil cp gs://que-me-pongo-jobs/que-me-pongo-job-alertas.jar .\ngcloud logging write batch-execution \"Se va a ejecutar el job de alertas\"\njava -jar que-me-pongo-job-alertas.jar\ngcloud logging write batch-execution \"Se ejecutó el job de alertas\"\ngcp_zone=$(curl -H Metadata-Flavor:Google http://metadata.google.internal/computeMetadata/v1/instance/zone -s | cut -d/ -f4)\ngcloud compute instances delete $(hostname) --zone ${gcp_zone}"
            }
        ]
    },
    "tags": {
...
exports.createInstance = (req, res) => {
    const vmName = "instancia-job-alertas-" + Date.now();
    var success = true
    try {
        compute.zone(zone)
