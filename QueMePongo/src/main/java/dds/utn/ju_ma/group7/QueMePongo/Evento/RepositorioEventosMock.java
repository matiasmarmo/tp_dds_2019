...
public class RepositorioEventosMock extends RepositorioEventos {
	...
	public List<Evento> eventosDelUsuario(Usuario usuario) {
		return this.todosLosEventos().stream().filter(evento -> evento.esDeUsuario(usuario))
				.collect(Collectors.toList());
	}
	...
	public Sugerencia obtenerSugerenciaDelUsuario(Usuario usuario, Long idSugerencia) {
		return this.eventosDelUsuario(usuario).stream().map(evento -> evento.getSugerencias())
				.flatMap(sugerencia -> sugerencia.stream())
				.filter(sugerencia -> sugerencia.esDichaSugerencia(idSugerencia)).findFirst().get();
	}
	...
	public List<Evento> obtenerEventosSugeridosDeUnGuardarropasParaFecha(Guardarropa guardarropa,
		...
		return this
				.todosLosEventos().stream().filter(evento -> evento.esEnFecha(fechaReferencia)
	...
	public List<EventoUnico> obtenerEventosEntreFechas(Calendar fechaInicio, Calendar fechaFin) {
		return this.todosLosEventos().stream()
	...
	public List<Sugerencia> sugerenciasCalificablesDelUsuario(Usuario usuario) {
		return this.eventosDelUsuario(usuario).stream().map(evento -> evento.getSugerencias())
				.flatMap(sugerencia -> sugerencia.stream())
				.filter(sugerencia -> sugerencia.esDeEstado(EstadoSugerencia.ACEPTADA)
						|| sugerencia.esDeEstado(EstadoSugerencia.ACEPTADA))
				.collect(Collectors.toList());
	}
