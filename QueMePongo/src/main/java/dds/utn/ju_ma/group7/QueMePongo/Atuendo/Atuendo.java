...
public class Atuendo {
	...
	public double interpolacionTemperatura(double temperatura) {
		...
		// P(x) = (-(19x^(6)))/(144000000)+ (43x^(5))/(4800000)- (23x^(4))/(288000)- (13x^(3))/(3200)+ (427x^(2))/(7200)- (71x)/(60)+60
		// (Atr matematica superior)
		return - 0.000000131 * Math.pow(temperatura, 6.0)
				+ 0.000008958 * Math.pow(temperatura, 5.0)
				- 0.000079861 * Math.pow(temperatura, 4.0)
				- 0.0040625 * Math.pow(temperatura, 3.0)
				+ 0.059305 * Math.pow(temperatura, 2.0)
				- 1.183333 * temperatura
				+ 60;
	}
	...
	private Map<ParteCuerpo, List<Prenda>> obtenerPrendasPorParteCuerpo() {
		...
		return partesCuerpo.stream().collect(
				Collectors.toMap(key -> key, key -> new ArrayList<Prenda>())
			);
	}
	
	private boolean prendasParaParteCuerpoAdecuadas(ParteCuerpo parteCuerpo, List<Prenda> prendas, double temperaturaInterpolada, Sensibilidad sensibilidad) {
		double nivelAbrigo = prendas.stream().mapToInt(prenda -> prenda.getNivelAbrigo()).sum();
		double abrigoAproximado = temperaturaInterpolada * parteCuerpo.getCoeficienteAbrigoMinimo() + sensibilidad.obtenerNivelSensibilidad(parteCuerpo);
