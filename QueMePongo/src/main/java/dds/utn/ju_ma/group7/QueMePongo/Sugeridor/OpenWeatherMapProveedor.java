...
public class OpenWeatherMapProveedor implements ProveedorClima{
	 ...
	 private JSONArray getPronosticoCincoDias() {
		ClientResponse response = this.client.resource(API)
				.queryParam("lat", lat_CABA).queryParam("lon", lon_CABA)
...
//		 formato de fecha en json: "2019-05-25 18:00:00"
		 ...
		 fechaCalendar.set(Integer.parseInt(campoAnio), Integer.parseInt(campoMes), Integer.parseInt(campoDia), Integer.parseInt(campoHora), 00);
	 ...
	 private boolean fechaCoincide(String fechaString, Calendar fechaBuscada) {
		 ...
		 return 
				 fechaCalendar.get(Calendar.YEAR) == fechaBuscada.get(Calendar.YEAR)
